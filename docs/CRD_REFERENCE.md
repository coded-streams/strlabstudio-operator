# FlinkSQLStudio CRD Reference

## Group / Version / Kind
`codedstreams.io/v1alpha1/FlinkSQLStudio`

## Spec Fields

| Field | Type | Required | Default | Description |
|---|---|---|---|---|
| `spec.image` | string | No | `codedstreams/flinksql-studio:latest` | IDE Docker image |
...
| `spec.service.nodePort` | integer | No | — | NodePort (30000–32767, only for NodePort type) |
...
- `Deployment/flinksql-studio-<name>` — the IDE pods
- `Service/flinksql-studio-<name>` — exposes the IDE

Both have `ownerReferences` pointing to the CR so they are garbage-collected when the CR is deleted.

## Short name
`kubectl get fss` is equivalent to `kubectl get flinksqlstudios`
